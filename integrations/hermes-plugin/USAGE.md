...
| 工具 | 作用 | 典型 LLM 触发词 |
|---|---|---|
| `ascent_synthesize` | Rust synthesize 产 `report.json`+`report.html`；surface loop 一直在写的 `session.md` 作 featured | "synthesize the report" / "finalize X" |
| `ascent_illustrate_hero` | actionbook 驱动 ChatGPT 走真实 GPT-Image-2，写 `images/hero.png`（**不**动 `session.md`——想引用自己加 markdown link） | "generate a hero cover for it" / "illustrate the report" |
...
**`session.md` 是一等输出**：v0.3 起 `synthesize` 不再链接 brief-md 渲染（derivative 短摘要丢 nuance）。Envelope `data.session_md` 指向 `<slug>/session.md`，loop 一直在写的那份。
...
Plugin handler (illustrate.py:generate_hero):
  1. _ensure_synthesized(slug) → report.json + session.md 都在?
  ...
  9. actionbook browser press Enter
 ...
 12. （不动 session.md）—— 用户想引用 hero 自己加 `![hero](images/hero.png)`
...
| `ascent_illustrate_hero` → `SESSION_MD_MISSING` | session 目录里 `session.md` 不在（corrupted）——查 `~/.actionbook/ascent-research/<slug>/` |
| `ascent_illustrate_hero` → `REPORT_JSON_MISSING` | 先跑 `ascent_synthesize` |
| `ascent_illustrate_hero` → `ACTIONBOOK_TIMEOUT` | research-local 太忙 / Chrome 卡顿：关些不用的 tab |
| `ascent_illustrate_hero` → `SESSION_CLOSED` | actionbook session 挂了：重开 Chrome |
...
| `ascent_illustrate_hero` → `IMAGE_MODE_ENTRY_FAILED` | `+ → Create image` 点击失败——ChatGPT 改 UI 了。inspect debug HTML 看菜单项新 selector，更新 `illustrate.py::SELECTORS["menu_create_image"]` 的 XPath |
