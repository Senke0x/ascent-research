...
| 工具 | 作用 | 典型 LLM 触发词 |
|---|---|---|
| `ascent_synthesize` | session → `report.json` + `report.html`（editorial report） | "synthesize the report" / "finalize X" |
| `ascent_loop_step` | autonomous loop 的 **一步**（读 SCHEMA + state → 选 action → 执行） | "run one research-loop step" |

**两个都是 LLM-backed**。`synthesize` 还能 `bilingual: true` 产中英双语报告（额外 LLM 消耗）。
